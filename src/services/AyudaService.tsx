...
import type { AyudaInteriorResponse, AyudaMateria, AyudaMateriasResponse, AyudaPayload, AyudaResponse, AyudaTemasResponse, AyudaTutor, AyudaTutoresResponse, AyudaTutorPayload, Temas } from "../types/ayuda.interface";
...
export const useCreateAyudaAlumnos = async (payload: AyudaPayload): Promise<AyudaResponse> => {
    payload = {...payload, telefono: payload.telefono.replace(/\D/g, "")};

    const encryptedPayload = await apiClient.encryptData({...payload});
    return await apiClient.post<AyudaResponse>(AYUDA_ENDPOINTS.POST_AYUDA_ALUMNOS.path, { data: encryptedPayload });
};

export const useCreateAyudaTutor = async (payload: AyudaTutorPayload): Promise<any> => {
    const encryptedPayload = await apiClient.encryptData({...payload});
    return await apiClient.post<any>(AYUDA_ENDPOINTS.POST_AYUDA_TUTOR.path, { data: encryptedPayload });
};
